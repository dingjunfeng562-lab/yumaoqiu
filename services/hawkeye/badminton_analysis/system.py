...
class BadmintonAnalysisSystem:
    def __init__(self, video_path, show_display=True, 
        ...
        self.force_court_view = force_court_view
        ...
        if self.pose_family == 'yolo-pose':
            ...
            try:
                ...
                raise RuntimeError(
                    f"Missing Python dependency for {self.pose_family}: {exc.name}. "
                    "Install RTMPose dependencies with: pip install rtmlib onnxruntime"
                ) from exc
            self.rtmpose_processor = _RTMPoseProcessor(mode=self.pose_mode, pose_family=self.pose_family)
    ...
    def _process_frame(self, frame, template_gray, corners, roi_corners, frame_count, out, detect_frame_count):
        ...
        is_court = self.force_court_view or self.is_court_view(gray_frame, template_gray)
        ...
        if not is_court:
            self._write_output_frame(frame, out, frame_count)
    ...
    def _write_output_frame(self, frame, out, frame_count):
        if frame is None:
            return
