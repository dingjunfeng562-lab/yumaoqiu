...
# Resolve project root so relative model/data paths work regardless of cwd
...
def detect_court(template_file, trigger):
    ...
    return preview_rgb, corners, trigger + 1
...
def apply_manual_corners(template_file, corners_state, trigger):
...
def build_ui():
    ...
    with gr.Blocks(
        ...
        detect_trigger = gr.State(value=0)
        ...
        detect_btn.click(
            fn=detect_court,
            inputs=[template_input, detect_trigger],
            outputs=[court_image, corners_state, detect_trigger],
        ).then(
        ...
        apply_btn.click(
            ...
            inputs=[template_input, click_corners_state, detect_trigger],
            outputs=[court_image, corners_state, detect_trigger],
        ).then(
...
if __name__ == "__main__":
    demo = build_ui()
    demo.queue(default_concurrency_limit=1).launch(server_name="0.0.0.0", server_port=7861, theme=gr.themes.Soft())
